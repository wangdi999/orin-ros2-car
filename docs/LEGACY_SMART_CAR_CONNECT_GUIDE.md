...
- Added `ohcar_wifi_profile.xml` for the manual's `ohcar` Wi-Fi profile.
...
- Wi-Fi password: set the lab/device password locally; do not commit it.
- VNC password: set the device password locally; do not commit it.
...
4. Create or use a Wi-Fi hotspot named `ohcar` with the local lab/device password.
...
10. Enter the local device VNC password.
...
- Password rejected: use the device VNC password, not the Wi-Fi password.
- Cannot find `ohcar`: check the two Wi-Fi antennas are installed, or create a phone hotspot named exactly `ohcar` with the local lab/device password.
