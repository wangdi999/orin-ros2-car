...
export default function App() {
  ...
  const [heartbeat, setHeartbeat] = useState({ seq: 0, lastAt: null, missed: 0 });
  ...
  const heartbeatOk = heartbeat.lastAt && Date.now() - heartbeat.lastAt < config.control.watchdogMs * 2;
  const aiDetections = showAiBoxes ? demoDetections() : [];
  ...
  useEffect(() => {
    if (!canDrive) {
      setHeartbeat((current) => ({ ...current, missed: current.lastAt ? current.missed + 1 : current.missed }));
      return undefined;
    }
    const interval = setInterval(() => {
      setHeartbeat((current) => ({ seq: current.seq + 1, lastAt: Date.now(), missed: 0 }));
      sendDrive({ forward: 0, turn: 0, strafe: 0 }, false);
    }, Math.max(180, config.control.watchdogMs / 2));
    return () => clearInterval(interval);
  }, [canDrive, config.control.watchdogMs, sendDrive]);
  ...
  const topMetrics = useMemo(() => ([
    ...
    { label: '心跳', value: heartbeatOk ? `${heartbeat.seq}` : '待命', tone: heartbeatOk ? 'green' : 'amber' }
  ]), [config.car.host, heartbeat.seq, heartbeatOk, state.runtime.rosbridge.connected, status, telemetry.voltage]);
        ...
        <section className="center-zone">
          <div className="visual-grid">
            <CameraPanel
              ...
              onToggleAiBoxes={() => setShowAiBoxes((value) => !value)}
              onGenerateAlert={() => setAlerts((items) => [createAlert(), ...items].slice(0, 8))}
            />
            <MapPanel alerts={alerts} driveVector={driveVector} />
          ...
          <div className="control-grid">
            ...
            <SpeedPanel
              ...
              disabled={!canDrive}
            />
            <HeartbeatPanel heartbeat={heartbeat} ok={heartbeatOk} watchdogMs={config.control.watchdogMs} canDrive={canDrive} />
            <AlertPanel alerts={alerts} onClear={() => setAlerts([])} onAck={(id) => setAlerts((items) => items.map((item) => item.id === id ? { ...item, status: '已确认' } : item))} />
          </div>
          <ServiceActions
            busy={busy}
...
function CameraPanel({ host, videoReady, detections, showAiBoxes, onToggleAiBoxes, onGenerateAlert }) {
    ...
    <section className="panel media-panel">
      <PanelTitle
        ...
        right={(
          <div className="toolbar-actions">
            <button onClick={onToggleAiBoxes}>{showAiBoxes ? '隐藏检测框' : '显示检测框'}</button>
            <button onClick={onGenerateAlert}>模拟报警</button>
          </div>
        )}
      />
      <div className="camera-frame">
        ...
        <div className="ai-overlay">
          {detections.map((item) => (
            <div
              className={`ai-box severity-${item.severity}`}
              key={item.id}
              style={{ left: item.x, top: item.y, width: item.w, height: item.h }}
            >
              <span>{item.type} {Math.round(item.confidence * 100)}%</span>
            </div>
          ))}
        </div>
...
function MapPanel({ alerts, driveVector }) {
  const route = [
    [14, 72],
    [34, 42],
    [58, 48],
  ...
  const robotX = 48 + driveVector.strafe * 10 + driveVector.turn * 4;
  const robotY = 62 - driveVector.forward * 10;

  return (
    <section className="panel map-panel">
      <PanelTitle title="地图显示" right={<div className="mode-pill">A-03</div>} />
      <div className="map-canvas">
        <svg viewBox="0 0 100 100" preserveAspectRatio="none">
          ...
          <rect width="100" height="100" className="map-grid" fill="url(#mapGrid)" />
          <polyline className="map-route" points={route.map(([x, y]) => `${x},${y}`).join(' ')} />
          {route.map(([x, y], index) => (
            <g key={`${x}-${y}`}>
              <circle className="map-point" cx={x} cy={y} r={index === 0 ? 2.8 : 2.1} />
              <text x={x + 2.4} y={y - 2}>{`P${index + 1}`}</text>
            </g>
          ))}
          {activeAlert && (
            <g className="map-alert">
              <circle cx="76" cy="36" r="4.6" />
              <text x="81" y="35">{activeAlert.type}</text>
            </g>
          )}
          <g className="map-robot">
            <circle cx={robotX} cy={robotY} r="4.2" />
            <path d={`M${robotX},${robotY - 7} L${robotX - 4},${robotY + 4} L${robotX + 4},${robotY + 4} Z`} />
          </g>
        </svg>
      </div>
      <div className="media-footer">
        <span>路线点 {route.length}</span>
        <span>{activeAlert ? `最新报警 ${activeAlert.zone}` : '无报警点'}</span>
      </div>
    </section>
  );
}

function HeartbeatPanel({ heartbeat, ok, watchdogMs, canDrive }) {
  const age = heartbeat.lastAt ? Date.now() - heartbeat.lastAt : null;
  return (
    <section className="panel heartbeat-panel">
      <PanelTitle title="心跳保护" right={<span className={ok ? 'ok-text' : 'warn-text'}>{ok ? 'ACTIVE' : 'STANDBY'}</span>} />
      <div className="heartbeat-core">
        <div className={`pulse-ring ${ok ? 'active' : ''}`}>
          <span />
        </div>
        <div>
          <strong>{heartbeat.seq}</strong>
          <span>heartbeat seq</span>
        </div>
      </div>
      <DataGrid rows={[
        ['发送周期', `${Math.max(180, watchdogMs / 2)} ms`],
        ['保护阈值', `${watchdogMs} ms`],
        ['最近心跳', age === null ? '未发送' : `${age} ms 前`],
        ['运动许可', canDrive ? '允许' : '锁定']
      ]} />
    </section>
  );
...
function AlertPanel({ alerts, onAck, onClear }) {
  return (
    <section className="panel alert-panel">
      <PanelTitle
        title="报警列表"
        right={<button onClick={onClear}>清空</button>}
      />
      <div className="alert-list">
        {alerts.length === 0 ? (
          <div className="empty-alert">暂无报警</div>
        ) : alerts.map((alert) => (
          <article className={`alert-card severity-${alert.severity}`} key={alert.id}>
            ...
            <p>{alert.zone} / 置信度 {Math.round(alert.confidence * 100)}%</p>
            <small>{alert.time}</small>
            <button onClick={() => onAck(alert.id)}>确认</button>
          </article>
        ))}
      </div>
    </section>
...
function demoDetections() {
  return [
    { id: 'box-obstacle', type: 'obstacle', confidence: 0.86, severity: 'high', x: '58%', y: '24%', w: '25%', h: '33%' },
    { id: 'box-edge', type: 'edge', confidence: 0.72, severity: 'medium', x: '18%', y: '62%', w: '42%', h: '14%' }
  ];
}
...
function createAlert() {
  const samples = [
    ['obstacle', 'A-03 巡检路段', 0.86, 'high'],
    ['flooding', 'B-02 低洼区', 0.78, 'medium'],
    ['pothole', 'C-01 主通道', 0.82, 'high']
  ];
  const [type, zone, confidence, severity] = samples[Math.floor(Math.random() * samples.length)];
  return {
    id: `${type}-${Date.now()}`,
    type,
    confidence,
